...
module.exports = async (req, res) => {
    ...
    '<head>' +
        ...
        '</style>' +
    '</head>';
    const standaloneAdminHtmlOutputBody = '<body>' +
    ...
    '<div class="master-layout">' +
        ...
        '</div>' +
    '</div>';
    const standaloneAdminHtmlOutputScript = '<script>' +
        'let localProductCacheMemory = [];' +
        'let base64ImagePayload = "";' +
        'var API_BASE_BASE = window.location.origin + window.location.pathname;' +
        'async function loadDashboardData() {' +
            'try {' +
                'var response = await fetch(API_BASE_BASE + "?action=fetch_all_data");' +
        ...
        'function renderProductsTable(products) {' +
            ...
            'products.forEach(function(p) {' +
                'var count = p.stockCount || 0;' +
                'var status = p.stockStatus || "OUTOFSTOCK";' +
                'rowsHtml += "<tr>" +' +
        ...
        'function renderOrdersTable(orders) {' +
            ...
            'orders.forEach(function(o) {' +
                'var itemsStr = o.items.map(function(i) { return i.name + " (x" + i.qty + ")"; }).join(", ");' +
                'var isFulfilled = o.status === "FULFILLED";' +
                'rowsHtml += "<tr style=\'background: " + (isFulfilled ? "#f8fafc" : "#ffffff") + ";\'>" +' +
                    ...
                    '"<td><strong style=\'display:block;\'>" + o.customerName + "</strong><span style=\'font-size:12px; color:#475569;\'' +
                    ' + \'><i class=\'fas fa-phone\'></i> " + o.customerMobile + "</span><br><span style=\'font-size:11px; color:#64748b;\'' +
                    ' + \'><i class=\'fas fa-map-marker-alt\'></i> " + o.customerAddress + "</span></td>" +' +
            ...
            'if (file) {' +
                ...
                'reader.onload = function(e) {' +
                    ...
                    'img.onload = function() {' +
                        ...
                        'var pv = document.getElementById("imagePreview");' +
        ...
        'function triggerLocalEdit(targetId) {' +
            'var product = localProductCacheMemory.find(function(item) { return item.id === targetId; });' +
            'if(!product) return;' +
        ...
        'document.getElementById("itemDeployForm").onsubmit = async function(e) {' +
            ...
            'var res = await fetch("/api/create-product", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });' +
        ...
        'async function processFulfillment(orderId) {' +
            ...
            'var res = await fetch("/api/fulfill-order", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ orderId: orderId }) });' +
            'var data = await res.json();' +
            'if(data.success) { alert(data.message); loadDashboardData(); }' +
        '}' +
        'function downloadInventoryCSV() {' +
            'var csv = "ID,Title,Category,Price,Stock\\n";' +
        ...
        'function downloadOrdersCSV() {' +
            'var csv = "OrderID,Total,Status\\n";' +
            'var rows = document.querySelectorAll("#orders-table-body tr");' +
    ...
    'res.status(200).send(standaloneAdminHtmlOutput + standaloneAdminHtmlOutputBody + standaloneAdminHtmlOutputScript);' +
'};
