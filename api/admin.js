// api/admin.js - Master Control Terminal Endpoint (UI + Data Gateway)
...
module.exports = async (req, res) => {
    // 🌟 1. BACKGROUND DATA GATEWAY: Fetches and saves array points safely
    if (req.query.loadData === 'true') {
        try {
    ...
    // 🌟 2. VISUAL DASHBOARD DESK SURFACE: Renders clean HTML template flawlessly
    const dynamicConsoleHtmlOutput = `<!DOCTYPE html>
    <html lang="en">
    <head>
        <meta charset="UTF-8">
    ...
    <script>
        ...
        // Dynamically tracks route URL queries to connect data layers securely
        const DATA_SYNC_PATH = window.location.origin + window.location.pathname + "?loadData=true";

        async function loadDashboardData() {
            try {
                const response = await fetch(DATA_SYNC_PATH);
        ...
        async function processFulfillment(orderId) {
            ...
            const res = await fetch('/api/fulfill-order', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ orderId: orderId }) });
            const data = await res.json();
        ...
        function downloadInventoryCSV() {
            ...
            localProductCacheMemory.forEach(p => csv += p.id + ',' + p.title.replace(/,/g,'') + ',' + p.category + ',' + p.currentPrice + ',' + p.stockCount + '\\n');
    ...
    res.status(200).send(dynamicConsoleHtmlOutput);
};
