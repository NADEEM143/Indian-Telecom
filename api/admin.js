...
module.exports = async (req, res) => {
    ...
    try {
        ...
        if (method === 'POST' && query.action === 'create-product') {
            ...
            const stockInitial = parseInt(stockCount) || 50;
...
<head>
    ...
    <style>
        ...
        .dropzone-box { border: 2px dashed #94a3b8; border-radius: 12px; padding: 24px; text-align: center; cursor: pointer; background: #f8fafc; position: relative; }
...
<script>
        ...
        files.forEach(file => {
            ...
            reader.onload = function(e) {
                ...
                img.onload = function() {
                    ...
                    // 🌟 LEAN RESOLUTION CHECK: Scales down dimensions aggressively to fit safely inside Vercel's 4.5MB ceiling boundaries
                    if (width > 450) { height *= 450 / width; width = 450; }
                    canvas.width = width; canvas.height = height;
                    ctx.drawImage(img, 0, 0, width, height);
                    
                    const singleBase64 = canvas.toDataURL('image/jpeg', 0.4);
    ...
    document.getElementById('itemDeployForm').onsubmit = async function(e) {
        ...
        submitButton.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Synchronizing...';
